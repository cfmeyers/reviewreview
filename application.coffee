...
$(document).ready ->
    ...
    $('#searchBoxGenre').typeahead(
        ...
        {name: 'genres', limit:15, source:substringMatcher(genres)}
    )

    $('#searchBoxTitle').typeahead(
        {hint: true, highlight: true, minLength: 1},
        {name: 'titles', limit:15, source:substringMatcher(titles)}
    )

    $('#searchBoxAuthor').typeahead(
        {hint: true, highlight: true, minLength: 1},
        {name: 'authors', limit:15, source:substringMatcher(authors)}
    )
...
titles = [
    "Launching The Innovation Renaissance: A New Path to Bring Smart Ideas to Market Fast (TED Books Book 8)"
    ...
    "A Great Leap Forward: 1930s Depression and U.S. Economic Growth (Yale Series in Economic and Financial History)"
